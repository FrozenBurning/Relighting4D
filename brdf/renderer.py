...
class SphereRenderer:
    ...
    def __init__(
        ...
        self.world2local = utils.geometry.normal.gen_world2local( # world-to-local
    ...
    def _calc_normals(self, eps=1e-12):
        ...
        normal = utils.linalg.normalize(normal, axis=2)
        # if self.debug:
    ...
    def gen_view_dir(self, local=False):
        ...
        vdir = utils.linalg.normalize(vdir, axis=2)
        return vdir
    ...
    def gen_light_dir(self, local=False):
        ...
        ldir = utils.linalg.normalize(ldir, axis=3)
        return ldir
...
def gen_light_xyz(envmap_h, envmap_w, envmap_radius=1e2):
    ...
    xyz = utils.geometry.sph.sph2cart(rlatlngs)
...
def load_light(envmap_path, envmap_inten=1., envmap_h=None, vis_path=None):
    ...
    if envmap_h is not None:
        ...
        h, w = envmap.shape[:2]
        interp = cv2.INTER_LINEAR if envmap_h > h else cv2.INTER_AREA
        envmap = cv2.resize(envmap, (int(envmap_h), int(envmap_h)), interpolation=interp)
