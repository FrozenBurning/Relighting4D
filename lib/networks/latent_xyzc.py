...
class Network(nn.Module):
    def __init__(self):
        ...
        self.num_vertices = 10475 if cfg.smplx else 6890
        self.c = nn.Embedding(self.num_vertices, 16)
    ...
    def encode_sparse_voxels(self, sp_input):
        ...
        code = self.c(torch.arange(0, self.num_vertices).to(coord.device))
    ...
    def pts_to_can_pts(self, pts, sp_input):
        ...
        if cfg.smplx:
            Th = sp_input['Th']
            R = sp_input['R']
            pts = torch.matmul(pts, torch.transpose(R, 1, 2)) + Th
        else:
            Th = sp_input['Th']
            pts = pts - Th
            R = sp_input['R']
    ...
    def get_feature(self, wpts, sp_input, feature_volume):
    ...
    def get_feature_density(self, wpts, sp_input):
        feature_volume = self.encode_sparse_voxels(sp_input)
        ppts = self.pts_to_can_pts(wpts, sp_input)
        grid_coords = self.get_grid_coords(ppts, sp_input)
        grid_coords = grid_coords[:, None, None]
        ...
        latent = latent[..., None].expand(*latent.shape, net.size(2))
        features = torch.cat((features, latent), dim=1)
        return features, self.alpha_fc(net)
    ...
    def forward(self, sp_input, grid_coords, viewdir, light_pts):
        ...
        code = self.c(torch.arange(0, self.num_vertices).to(p_features.device))
        xyzc = spconv.SparseConvTensor(code, coord, out_sh, batch_size)
